...
export const FamilyChatDrawer: React.FC<FamilyChatDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      <div className="bg-white border border-slate-200 w-full max-w-md h-[85vh] sm:max-h-[90vh] rounded-3xl flex flex-col shadow-2xl relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden">
        <div className="p-5 sm:p-6 flex flex-col h-full">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-4 shrink-0">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <h3 className="text-base font-extrabold text-slate-900 leading-tight">{member.name}</h3>
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-4 text-xs scrollbar-thin">
            {messages.map((msg) => (
              ...
              >
                <span className="text-[10px] text-slate-500 font-medium mb-1 px-1">{msg.senderName} • {msg.timestamp}</span>
                <div
          ...
          {/* INPUT FORM */}
          <form onSubmit={handleSendMessage} className="pt-4 border-t border-slate-100 flex items-center gap-2 shrink-0">
            <input
              type="text"
              value={inputMessage}
              onChange={(e) => setInputMessage(e.target.value)}
              placeholder={`Message ${member.name}...`}
              className="flex-1 px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs font-medium focus:outline-none focus:border-[#00a896] focus:ring-1 focus:ring-[#00a896] transition-all"
            />
            <button
              type="submit"

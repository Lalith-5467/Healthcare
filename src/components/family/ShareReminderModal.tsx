...
import { X, Pill, Check, Sparkles } from 'lucide-react';
import type { FamilyMember, SharedReminder } from './familyData';
...
export const ShareReminderModal: React.FC<ShareReminderModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newRem: SharedReminder = {
      id: `REM-SH-${Date.now().toString().slice(-4)}`,
      title: reminderTitle,
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-6 shadow-2xl relative">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-extrabold text-slate-900 dark:text-white">Share Medication Reminder</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Select family members to share routine dose alerts</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          <div>
            <label className="block text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-1.5">Reminder Name</label>
            <input
              type="text"
              value={reminderTitle}
              ...
              {members.map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    {selected && <Check className="w-4 h-4 text-[#00a896]" />}
                  </button>
                );
              })}
            </div>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-5 py-2.5 rounded-xl font-extrabold text-slate-900 dark:text-white bg-gradient-to-r from-[#00a896] to-cyan-600 hover:from-teal-600 hover:to-cyan-500 transition-all shadow-lg flex items-center gap-2 cursor-pointer disabled:opacity-75"
            >
              {saving ? (
                <>
                  <Sparkles className="w-4 h-4 animate-spin" />
                  <span>Sharing Reminder...</span>
                </>
              ) : (
                <>

      ...
      <div className="bg-white border border-slate-200 rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-6 shadow-2xl relative">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-bold uppercase tracking-wider text-teal-600">Step {step} of 2</span>
        ...
        {sentSuccess ? (
          <div className="py-6 text-center space-y-4">
            ...
            <div>
              <h4 className="text-base font-extrabold text-slate-900">Invitation Sent Successfully</h4>
              <p className="text-xs text-slate-500 mt-1">
                Your family member <strong className="text-slate-900">{fullName}</strong> ({relationship}) has been added to pending connections.
              </p>
            </div>
            <button
              onClick={handleDone}
            ...
            {step === 1 ? (
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-slate-600 font-bold uppercase tracking-wider mb-1.5">Relationship</label>
                    <select
                      value={relationship}
                      onChange={(e) => setRelationship(e.target.value as any)}
                      className="w-full px-3 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-semibold focus:outline-none focus:border-[#00a896]"
                    >
                  ...
                  <div>
                    <label className="block text-slate-600 font-bold uppercase tracking-wider mb-1.5">Age (Optional)</label>
                    <input
                      type="number"
                      value={age}
                      onChange={(e) => setAge(e.target.value)}
                      placeholder="e.g. 58"
                      className="w-full px-3 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-mono font-bold focus:outline-none focus:border-[#00a896]"
                    />
                  </div>
                </div>
              ...
              <>
                ...
                <div className="p-3.5 rounded-2xl bg-cyan-50 border border-cyan-100 text-[11px] text-cyan-700 font-medium">
                ...
                <div className="pt-4 border-t border-slate-100 flex justify-between">
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="py-2.5 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-semibold cursor-pointer"
                  >
                    Back
                  </button>
                  <button

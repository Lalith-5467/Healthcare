      ...
      >
        ...
        <div className={`shrink-0 transition-transform duration-200 group-hover:scale-110 ${
          isActive 
            ...
            : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-600 dark:text-slate-300'
        }`}>
        ...
        {!isCollapsed && (
          <div className="flex-1 flex items-center justify-between min-w-0 transition-all duration-150">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {/* AI BADGE */}
              {isAI && (
                <span className="px-1.5 py-0.5 text-[9px] font-black tracking-wider uppercase bg-gradient-to-r from-purple-500 to-indigo-500 text-slate-900 dark:text-white rounded shadow-sm">
                  AI
                </span>
              )}
              ...
              {hasChildren && (
                <ChevronDown className={`w-3.5 h-3.5 text-slate-500 dark:text-slate-400 transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} />
              )}
            </div>
          </div>
        )}

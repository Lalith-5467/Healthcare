...
export const ABHASection: React.FC<ABHASectionProps> = ({ onManageConnection }) => {
  ...
  return (
    <section id="abha" className="py-24 bg-slate-900 text-white relative overflow-hidden">
      {/* GLOW DECORATIONS */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-base sm:text-lg text-slate-400 mt-4 leading-relaxed">
            PulseCare integrates with Ayushman Bharat Digital Mission (ABDM). Your ABHA address works as your key to authorize and receive health records from participating healthcare providers nationwide.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16 relative">
          {workflowSteps.map((item, idx) => {
            ...
            return (
              <div key={idx} className="relative p-6 rounded-2xl bg-slate-800/80 border border-slate-700/80 space-y-3 backdrop-blur-xl">
                ...
                {idx < 3 && (
                  <div className="hidden md:block absolute -right-3 top-1/2 -translate-y-1/2 z-20 bg-slate-800 p-1 rounded-full text-slate-500 border border-slate-700">
        ...
        <div className="max-w-2xl mx-auto p-8 rounded-3xl bg-gradient-to-r from-slate-800 via-slate-850 to-slate-900 border border-slate-700/80 shadow-2xl space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-700/80 pb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">ABDM Integration Status: Linked & Verified</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800">
              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold mb-1">ABHA Number</p>
              <p className="font-mono text-lg font-bold text-orange-400 tracking-wider">
                14-XXXX-XXXX-8921
              </p>
            </div>
            <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800">
              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold mb-1">ABHA Address</p>
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-xs text-slate-400">
              PulseCare routes authorization requests strictly through official ABDM gateway protocols.
            </p>

            <button
              onClick={onManageConnection}
              className="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 rounded-full text-xs font-bold text-white bg-[#FF5B22] hover:bg-[#e54c15] shadow-md shadow-orange-500/20 transition-all gap-2 shrink-0"
            >

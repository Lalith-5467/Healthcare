...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  const handleSaveGoal = (newGoal: Partial<HealthGoal>) => {
    const created: HealthGoal = {
      id: newGoal.id || `GOAL-${Date.now().toString().slice(-4)}`,
      title: newGoal.title || 'Personal Target',
      category: newGoal.category || 'Other',
      target: newGoal.target || 100,
      current: 0,
      unit: newGoal.unit || 'units',
      startDate: newGoal.startDate || '24 Aug 2026',
      endDate: newGoal.endDate || '24 Sep 2026',
      progress: 0,
      isPaused: false
    };
  ...
  const filteredActivities = activities.filter((act) => {
    ...
    if (filters.status !== 'All' && act.status !== filters.status) return false;
    if (searchQuery) {
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300 pb-16">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
        <div className="space-y-3 text-center md:text-left flex-1">
          ...
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white">Your health activity at a glance</h2>
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 pt-2">
            <span className="px-3 py-1 rounded-full bg-slate-950 text-slate-400 text-[11px] font-bold border border-slate-800">
              Period: {globalDateRange}
            </span>
            <span className="px-3 py-1 rounded-full bg-amber-500/10 text-amber-300 text-[11px] font-bold border border-amber-500/20">
              Demo Data Mode
            </span>
          </div>
        </div>
        ...
        <div className="bg-slate-950 p-6 rounded-3xl border border-slate-800 text-center space-y-2 shrink-0 min-w-[220px]">
          <div className="relative w-32 h-32 mx-auto flex items-center justify-center">
            ...
            <div className="absolute flex flex-col items-center justify-center">
              <span className="font-mono text-3xl font-extrabold text-white">86</span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-3xl space-y-2 shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400">Medication Adherence</span>
            <Pill className="w-4 h-4 text-amber-400" />
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl sm:text-3xl font-extrabold text-amber-400 font-mono">91%</span>
            <span className="text-[10px] font-bold text-emerald-400">+5.2%</span>
          </div>
          <span className="text-[10px] text-slate-400 block">vs previous period</span>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-3xl space-y-2 shadow-md">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-2xl sm:text-3xl font-extrabold text-cyan-400 font-mono">8</span>
            <span className="text-[10px] font-bold text-slate-400">visits</span>
          </div>
          <span className="text-[10px] text-slate-400 block">3 upcoming • 5 done</span>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-3xl space-y-2 shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400">Medical Records</span>
            <FileText className="w-4 h-4 text-purple-400" />
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl sm:text-3xl font-extrabold text-purple-300 font-mono">24</span>
            <span className="text-[10px] font-bold text-slate-400">records</span>
          </div>
          <span className="text-[10px] text-slate-400 block">Lab reports & prescriptions</span>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-3xl space-y-2 shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400">Consultations</span>
            <Video className="w-4 h-4 text-teal-400" />
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl sm:text-3xl font-extrabold text-teal-400 font-mono">12</span>
            <span className="text-[10px] font-bold text-slate-400">sessions</span>
          </div>
          <span className="text-[10px] text-slate-400 block">18 min average call</span>
        </div>
      </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-extrabold text-white">Vitals Overview</h3>
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30">
                Demo / manually entered values
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">Track heart rate, blood pressure & vital metrics</p>
          </div>
            ...
            {(['7D', '30D', '3M'] as const).map((tf) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                  vitalsTimeframe === tf ? 'bg-[#00a896] text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                {tf}
              </button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 text-xs">
          {[
            { id: 'Heart Rate', label: 'Heart Rate', val: '72 BPM', icon: Heart, color: 'text-rose-400' },
            { id: 'Blood Pressure', label: 'Blood Pressure', val: '120/80', icon: Activity, color: 'text-cyan-400' },
            { id: 'Temperature', label: 'Temperature', val: '98.4°F', icon: Thermometer, color: 'text-amber-400' },
            { id: 'SpO2', label: 'SpO₂ Oxygen', val: '98%', icon: Wind, color: 'text-teal-400' },
            { id: 'Weight', label: 'Body Weight', val: '68 kg (-1.2)', icon: Scale, color: 'text-purple-400' }
          ].map((m) => {
            const Icon = m.icon;
            ...
            return (
              <button
                ...
                className={`p-3 rounded-2xl border text-left transition-all cursor-pointer space-y-1 ${
                  isSelected
                    ? 'bg-slate-800 border-teal-400 shadow-md'
              ...
              >
                ...
                <span className="font-mono font-extrabold text-sm text-white block">{m.val}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 space-y-4">
          <div className="flex items-center justify-between text-xs">
            <span className="font-bold text-white">{selectedVitalMetric} Trend Line</span>
            <span className="text-[10px] font-mono text-slate-400">Interactive Preview</span>
          </div>

          <div className="h-40 flex items-end justify-between gap-3 pt-6 px-2 relative border-b border-slate-800">
              ...
              return (
                ...
                >
                  <motion.div
                    ...
                    className="w-full max-w-[28px] rounded-t-xl bg-gradient-to-t from-[#00a896] to-cyan-400 group-hover:from-teal-500 group-hover:to-cyan-300 transition-all shadow-md"
                  />
          ...
          {hoveredVital && (
            <div className="bg-slate-900 p-3 rounded-xl border border-slate-700 text-xs font-mono flex items-center justify-between text-cyan-300">
              <span>{hoveredVital.date} Data Point:</span>
        ...
        <div className="lg:col-span-6 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-xl">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-extrabold text-white">Medication Adherence</h3>
            </div>
            <button
          ...
          <div className="grid grid-cols-3 gap-3 bg-slate-950 p-4 rounded-2xl border border-slate-800 text-center text-xs">
            <div>
              ...
              <span className="font-mono font-extrabold text-emerald-400 text-lg">75%</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">This Week</span>
              <span className="font-mono font-extrabold text-cyan-400 text-lg">88%</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">This Month</span>
              <span className="font-mono font-extrabold text-teal-400 text-lg">91%</span>
            </div>
          </div>

          {/* WEEKLY ADHERENCE BARS */}
          <div className="space-y-2">
            <span className="text-xs font-bold text-slate-300">Weekly Adherence Log</span>
            <div className="grid grid-cols-7 gap-2 items-end h-28 bg-slate-950 p-3 rounded-2xl border border-slate-800">
              {MOCK_WEEKLY_ADHERENCE.map((day) => (
                <div key={day.day} className="flex flex-col items-center gap-1 h-full justify-end">
                  <div
                    style={{ height: `${day.percentage}%` }}
                    className="w-full max-w-[20px] rounded-t-lg bg-teal-400/80"
                  />
        ...
        <div className="lg:col-span-6 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-xl">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-extrabold text-white">Appointments & Consultations</h3>
            </div>
            <button
          ...
          <div className="grid grid-cols-3 gap-3 bg-slate-950 p-4 rounded-2xl border border-slate-800 text-center text-xs">
            <div>
              ...
              <span className="font-mono font-extrabold text-emerald-400 text-lg">12</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">Upcoming</span>
              <span className="font-mono font-extrabold text-cyan-400 text-lg">3</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">Cancelled</span>
              <span className="font-mono font-extrabold text-rose-400 text-lg">3</span>
            </div>
          </div>

          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3 text-xs">
            <h4 className="font-bold text-white">Consultation Summary</h4>
            <div className="space-y-2 text-slate-300">
              <div className="flex justify-between"><span>Total Tele-Consultations:</span><span className="font-bold text-white">12 sessions</span></div>
              <div className="flex justify-between"><span>Video Calls:</span><span className="font-bold text-teal-400">8 calls</span></div>
              <div className="flex justify-between"><span>In-Person Visits:</span><span className="font-bold text-cyan-400">4 visits</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            <h3 className="text-base font-extrabold text-white">Health Activity Timeline & Calendar</h3>
            <p className="text-xs text-slate-400">Search activity events or filter by date on the monthly calendar</p>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button
              ...
              className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-purple-400 border border-slate-700 cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 space-y-3">
            {filteredActivities.map((act) => (
              <div key={act.id} className="p-3.5 bg-slate-950 rounded-2xl border border-slate-800 flex items-center justify-between gap-3 text-xs">
                ...
                <div className="text-right shrink-0 font-mono">
                  ...
                  <span className="text-[10px] font-bold text-teal-400">{act.time}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="lg:col-span-5 bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="font-mono text-[10px] text-cyan-400 font-bold">{selectedCalendarDate}</span>
            </div>

            <div className="grid grid-cols-7 gap-1 text-center text-[10px]">
              {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, idx) => (
                <span key={idx} className="font-bold text-slate-400 py-0.5">{d}</span>
              ))}
              {Array.from({ length: 31 }, (_, i) => i + 1).map((d) => {
                ...
                return (
                  ...
                  >
                    <span>{d}</span>
                    {hasDot && <span className="w-1 h-1 rounded-full bg-amber-400 mt-0.5" />}
                  </button>
                );
              })}
            </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-extrabold text-white">Personal Health Goals</h3>
              <p className="text-xs text-slate-400">User-configured routine target goals</p>
            </div>
          </div>

          <button
            onClick={() => setCreateGoalModalOpen(true)}
            className="px-4 py-2.5 rounded-xl font-extrabold text-xs text-white bg-gradient-to-r from-[#00a896] to-cyan-600 hover:from-teal-600 hover:to-cyan-500 transition-all shadow-md flex items-center gap-2 cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {goals.map((g) => (
            <div key={g.id} className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-3 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-xs font-extrabold text-white">{g.title}</span>
                <span className="text-[10px] font-mono font-bold text-teal-400">{g.progress}%</span>
              </div>

              <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                <div style={{ width: `${g.progress}%` }} className="h-full bg-gradient-to-r from-[#00a896] to-cyan-400" />
              </div>

              <div className="flex justify-between items-baseline text-[11px]">
                <span className="text-slate-400">Target: <strong className="text-white">{g.target} {g.unit}</strong></span>
                <span className="text-slate-400">Current: <strong className="text-cyan-300">{g.current}</strong></span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 space-y-4 shadow-xl">
          <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
            ...
            <h3 className="text-base font-extrabold text-white">Health Activity Insights</h3>
          </div>

          <div className="space-y-3 text-xs">
            <div className="p-3.5 rounded-2xl bg-slate-800/40 border border-slate-800 space-y-1">
              <h4 className="font-bold text-cyan-300">Medication Routine Tracking</h4>
              <p className="text-slate-300">Medication tracking was completed on 6 of the last 7 days.</p>
            </div>
            <div className="p-3.5 rounded-2xl bg-slate-800/40 border border-slate-800 space-y-1">
              <h4 className="font-bold text-teal-300">Appointment Activity</h4>
              <p className="text-slate-300">Your appointment activity increased compared with the previous period.</p>
            </div>
            <div className="p-3.5 rounded-2xl bg-slate-800/40 border border-slate-800 space-y-1">
              <h4 className="font-bold text-purple-300">Medical Records Synced</h4>
              <p className="text-slate-300">You added 4 medical records to your health journal this month.</p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 space-y-4 shadow-xl">
          <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
            ...
            <h3 className="text-base font-extrabold text-white">This Month vs Previous Month</h3>
          </div>
          ...
          <div className="space-y-2.5 text-xs">
            {MOCK_MONTHLY_COMPARISONS.map((comp, i) => (
              <div key={i} className="p-3 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-white">{comp.label}</h4>
                  <span className="text-[10px] text-slate-400">Current: {comp.currentValue} vs {comp.previousValue}</span>
                </div>
                <span className="font-mono font-extrabold text-emerald-400 text-xs bg-emerald-500/10 px-2 py-1 rounded-lg border border-emerald-500/20">
                  {comp.change}
                </span>
              </div>
            ))}

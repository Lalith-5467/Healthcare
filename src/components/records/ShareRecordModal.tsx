      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-extrabold text-white">Share Medical Record</h3>
              <p className="text-xs text-slate-400 truncate max-w-[240px]">{record.title}</p>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-2 text-xs">
            ...
            <div className="p-3.5 rounded-2xl bg-slate-800 border border-slate-700 space-y-2">
              <span className="font-bold text-slate-300 block">Encrypted Access Link</span>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  readOnly
                  value={`https://health.abdm.in/records/share?id=${record.id}`}
                  className="flex-1 px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 font-mono text-[11px] select-all focus:outline-none"
                />
                <button
                  onClick={handleCopyLink}
                  className="px-3 py-2 rounded-xl bg-[#00a896] hover:bg-[#00897b] text-white font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0"
                >
            ...
            <button
              ...
              className="w-full p-3.5 rounded-2xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold transition-all flex items-center justify-between cursor-pointer group"
            >

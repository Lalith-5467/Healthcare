...
import { Files, FlaskConical, Pill, Stethoscope, FileImage, Syringe } from 'lucide-react';
...
export const RecordSummaryCards: React.FC<RecordSummaryCardsProps> = ({
  ...
  const cards = [
    ...
    { key: 'Lab Reports', label: 'Lab Reports', count: counts.Lab || 8, icon: FlaskConical, color: 'text-purple-400 bg-purple-500/10 border-purple-500/20' },
    { key: 'Prescriptions', label: 'Prescriptions', count: counts.Prescription || 6, icon: Pill, color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
    { key: 'Consultations', label: 'Consultations', count: counts.Consultation || 5, icon: Stethoscope, color: 'text-teal-400 bg-teal-500/10 border-teal-500/20' },

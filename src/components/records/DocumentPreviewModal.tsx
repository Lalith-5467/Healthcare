    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md">
        <motion.div
          ...
          className="w-full max-w-2xl max-h-[90vh] bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl text-white flex flex-col overflow-hidden relative"
        >
          {/* TOP BAR */}
          <div className="p-5 border-b border-slate-800 flex items-center justify-between gap-3 bg-slate-900/90">
            <div className="flex items-center gap-3 min-w-0">
              <div className="p-2 rounded-xl bg-[#00a896]/20 text-cyan-300 border border-teal-500/30">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-2 rounded-xl border text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                  showDetails ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30' : 'bg-slate-800 text-slate-300 border-slate-700'
                }`}
          ...
          {/* DOCUMENT BODY */}
          <div className="p-6 space-y-6 overflow-y-auto flex-1 bg-slate-950/40 font-sans">
            {/* SIMULATED OFFICIAL CLINICAL DOCUMENT SHEET */}
            <div className="p-6 sm:p-8 rounded-2xl bg-white text-slate-900 shadow-2xl border border-slate-200 space-y-6">
              {/* DOCUMENT HEADER BANNER */}
              <div className="flex items-start justify-between pb-4 border-b border-slate-200">
                <div>
                  <h4 className="text-lg font-black tracking-tight text-slate-900 uppercase">
                    {record.hospital}
                  </h4>
                  <p className="text-xs text-slate-500 font-semibold">Clinical Diagnostics & Record Department</p>
                </div>
                <div className="text-right">
                  <span className="text-xs font-mono font-bold text-slate-600 block">{record.id}</span>
                  <span className="text-[10px] font-bold uppercase text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3.5 rounded-xl bg-slate-50 border border-slate-200 text-xs">
                ...
                <div>
                  ...
                  <span className="font-bold text-slate-900">{record.doctor}</span>
                </div>
                <div>
              ...
              {record.metrics && record.metrics.length > 0 ? (
                      ...
                      <tbody className="divide-y divide-slate-200">
                        {record.metrics.map((m, idx) => (
                          <tr key={idx} className="hover:bg-slate-50">
                            ...
                            <td className="p-2.5 font-bold text-slate-900">{m.value}</td>
                            <td className="p-2.5 text-slate-500">Standard Adult Range</td>
                            <td className="p-2.5 text-right">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                                m.status === 'High' ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'
                              }`}>
                                {m.status}
                              </span>
                            </td>
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs space-y-2">
                  <h5 className="font-extrabold text-slate-800 uppercase tracking-wider">Clinical Notes & Impression</h5>
            ...
            {showDetails && (
              <motion.div
                ...
                className="p-4 rounded-2xl bg-slate-900 border border-slate-800 text-xs space-y-2"
              >
                <h4 className="font-extrabold text-cyan-300 uppercase tracking-wider">Document Metadata Details</h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-slate-300">
                  <div><span className="text-slate-500 block">Record ID:</span> {record.id}</div>
                  <div><span className="text-slate-500 block">File Name:</span> {record.fileName}</div>
                  <div><span className="text-slate-500 block">File Size:</span> {record.fileSize}</div>
                  <div><span className="text-slate-500 block">Status:</span> {record.status}</div>
                </div>
              </motion.div>
            )}
          </div>
          ...
          <div className="p-4 border-t border-slate-800 bg-slate-900 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 border border-indigo-500/30 font-bold text-xs transition-all flex items-center gap-1.5 cursor-pointer"
              >

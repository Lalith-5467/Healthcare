      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-[#0f172a] border border-slate-200/80 dark:border-slate-800 shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[11px] text-slate-400">{surgeries.length} Recorded Procedures</span>
            </div>
          </div>
        ...
        {surgeries.length === 0 ? (
          <div className="p-6 text-center rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-dashed border-slate-300 dark:border-slate-800 space-y-2">
            <p className="text-xs text-slate-400 font-semibold">No previous surgeries recorded</p>
            <button
              onClick={() => setModalOpen(true)}
            ...
            {surgeries.map((s) => (
              ...
              >
                <div className="space-y-1 min-w-0">
                  ...
                  {s.doctor && (
                    <p className="text-[10px] text-slate-400">Surgeon: {s.doctor}</p>
                  )}
                </div>

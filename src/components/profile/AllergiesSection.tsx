      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-[#0f172a] border border-slate-200/80 dark:border-slate-800 shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[11px] text-slate-400">{allergies.length} Recorded Allergies</span>
            </div>
          </div>

          <button
            onClick={() => setModalOpen(true)}
            className="px-3.5 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/20 text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer"
          >
        ...
        {allergies.length === 0 ? (
          <div className="p-6 text-center rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-dashed border-slate-300 dark:border-slate-800 space-y-2">
            <p className="text-xs text-slate-400 font-semibold">No allergies recorded</p>
            <button
              onClick={() => setModalOpen(true)}
                ...
                >
                  <div className="space-y-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      Reaction: <strong className="font-semibold">{a.reaction}</strong>
                    </p>
                    {a.notes && (
                      <p className="text-[10px] text-slate-400 italic">"{a.notes}"</p>
                    )}
                  </div>
